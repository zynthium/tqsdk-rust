...
fn describe_frame(frame: &tqsdk_core::RawFrame) -> String {
    match frame {
        tqsdk_core::RawFrame::Text(text) => {
        ...
        tqsdk_core::RawFrame::Binary(bytes) => format!("binary:{}bytes", bytes.len()),
        tqsdk_core::RawFrame::Ping => "ping".to_string(),
        tqsdk_core::RawFrame::Pong => "pong".to_string(),
        tqsdk_core::RawFrame::Close => "close".to_string(),
    }
}
