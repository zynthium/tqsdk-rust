    ...
    run_on_tokio(async {
        ...
        let market_id = handle
            .submit(RuntimeCommand::Market(MarketCommand::SubscribeQuotes {
                symbols: vec![tqsdk_core::Symbol::new(test_symbol.clone())],
            }))
            .await
...
async fn pump_route_until<T, F>(
    ...
    run: &mut tqsdk_core::SessionRun,
    route_label: &str,
    timeout: Duration,
    caused_by: Vec<tqsdk_core::CommandId>,
    scope: CommitScope,
    mut success: F,
) -> Result<Vec<String>, Vec<String>>
where
    F: FnMut(&tqsdk_core::StateSnapshot) -> Option<T>,
{
