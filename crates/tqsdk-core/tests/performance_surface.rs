...
#[test]
fn path_dispatcher_reuses_candidate_scratch_buffers() {
    let source = include_str!("../../tqsdk-stream/src/path_dispatcher.rs");
    let registry = function_block(
        source,
    ...
    assert!(
        registry.contains("candidate_ids: Vec<usize>"),
        "path dispatcher should retain reusable candidate-id storage on the registry"
    );
    assert!(
        registry.contains("candidate_seen: HashSet<usize>"),
        "path dispatcher should retain reusable candidate de-dup storage on the registry"
    );
    assert!(
        registry.contains("dead_ids: Vec<usize>"),
    ...
    assert!(
        !collector.contains("Vec::new()"),
        "candidate collection should clear and reuse the registry Vec instead of allocating per commit"
    );
    assert!(
        collector.contains("candidate_seen.clear()") && collector.contains("candidate_ids.clear()"),
        "candidate collection should explicitly clear reusable scratch buffers"
    );
}
