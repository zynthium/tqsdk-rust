...
//! `.server_backtest(start_ns, end_ns)`. Trade targets and automatic trade login
//! are intentionally rejected in this mode; use `.backtest(...).cache(...)` or
...
fn build_with_custom_replay_endpoint(
    ...
    Ok(Tq::futures()
        ...
        .server_backtest(start_ns, end_ns))
}
