...
use tqsdk_core::{Kline, MarketChartCommand, MarketCommand, RuntimeCommand, Symbol, Tick};
    ...
    pub fn build(self) -> Result<DataClient> {
        ...
        if self.history_cache_enabled {
            let cache = if let Some(dir) = self.history_cache_dir {
                ...
                HistorySeriesCache::python_compatible_default()
            }?;
...
fn next_data_series_chart_id(kind: &str, symbol: &str, duration_ns: i64) -> String {
    let sequence = NEXT_HISTORY_CHART_ID.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
    format!(
