    ...
    fn parse(value: &str) -> Result<Self> {
        ...
        if let Some(rest) = value.strip_prefix("top:") {
            let (limit, values) = rest
                .split_once(':')
                .ok_or_else(|| invalid_universe("top selector must be top:N:values"))?;
            let limit = limit.trim().parse::<usize>().map_err(|err| {
                invalid_universe(format!("top selector limit must be positive: {err}"))
            })?;
            if limit == 0 {
                return Err(invalid_universe(
...
fn parse_values(value: &str) -> Result<Vec<String>> {
    let value = value.trim();
    if value.is_empty() {
        return Err(invalid_universe(
