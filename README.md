...
- 对固定 root 做历史 tick 或 canonical-minute cache 预检、补齐、验收，或导出 cache-backed Tick/Kline 区间给脚本或模型：使用可选 `tqsdk-cache` CLI；普通策略不需要启动它。
...
`tqllm-csv/1`；详情见 [`tqsdk-cache` README](crates/tqsdk-cache/README.md)。
