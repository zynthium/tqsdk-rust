...
- `tqsdk-stream` 已落地最小 commit-stream facade，当前提供共享 session 驱动、raw commit fan-out、显式 lag/closed/error surface、path/scope/domain/object/field 级 commit 过滤，以及 typed market/system/trade/risk/settlement/security 单对象 stream；后续继续补 serial/window 与 trade 可靠事件流。
