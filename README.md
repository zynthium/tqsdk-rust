...
路径会稳定获取全物理合约 roster 与 metadata；V2 timeline 会在日线请求前按其 normalized AST
...
做有退避的小批 native-daily 重试。它仍会重新发现全量 roster/metadata 以验证 pinned scope，先做
provider-health canary，timeout 仅推进独立 retry receipt；
...
Universe V2 可用 `except(...)` 合并多个排除项：`except(contract:CFFEX.*,CZCE.ZC,CZCE.CY)` 只排 contract view，`except(all:CFFEX.*,CZCE.ZC,CZCE.CY)` 排所有可分类 view。同交易所 target 可缩写为 `CZCE.{ZC,CY,RI,RS}`；parser 会展开为既有逐项 target。两种写法都会规范化为既有 `!` 形式，故不会改变 canonical AST/hash 或已发布历史 artifact identity。
