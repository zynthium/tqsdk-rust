...
- 普通策略优先使用默认 `tqsdk` facade。`.backtest(start_ns, end_ns).cache_dir(...)` 走持久 tick cache-backed 本地撮合回测，默认 `RemoteOnMiss` 可用官方 server-side backtest 流补缺口；`.market_cache(policy)` 可把同一份 cache policy 用于 live recording 和 backtest 输入；`.replay_backtest(replay)` 走 caller-owned replay source + `TqSim`。策略主体保持 `Tq::next()` / `quote()`，live/backtest 差异留在 builder。契约锚点：S37-S39、S43-S47。
