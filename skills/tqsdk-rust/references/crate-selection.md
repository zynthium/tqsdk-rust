...
| Strategy backtest | `tqsdk` first; `tqsdk-wait` or `tqsdk-task + tqsdk-data` when explicit | 普通 live/backtest 同主体策略用默认 facade 的 `.backtest(...).cache_dir(...)` 或 `.replay_backtest(...)`；明确 Python-style wait builder 时用 `TqApiBuilder::futures_backtest`；本地确定性 `TqSim` 内部能力用 `StrategyBacktest` + `ReplayMarketSource`，历史 rows 可由 `tqsdk-data` 提供 |
