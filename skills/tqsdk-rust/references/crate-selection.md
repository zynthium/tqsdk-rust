...
- `tqsdk-stream` 负责 multi-consumer commit/event streams、filters、lag diagnostics 和 stream sinks。它可以暴露 `session()`，但不能变成 metadata/query 层。
...
- `tqsdk-data` 负责 research/offline data、history pages/series/downloads、CSV export、Python-compatible history cache、option Greeks 和 market-cache replay materialization。
