...
| "历史 HTTP", "history API", "`/v1/history/query`", "coverage", "schema", "Docker 部署 relay" | Read-only CacheOnly history service | `tqsdk-relay` over the actively filled cache | 先读 [`relay-history-http.md`](relay-history-http.md)；调用 `GET /v1/history/{query,coverage,schema}`，默认通过 `deploy/docker` 只读挂载 `TQSDK_LIVE_CACHE_ROOT`，无需发布快照或重启即可读取最新原子提交进度 |
