...
| "缓存盘点", "cache inventory", "补历史缓存", "cache fill", "cache verify", "cache doctor" | Historical tick cache operator workflow | optional `tqsdk-cache` binary | `inventory`, `inspect`, closed-day `fill`, `verify --report`, `doctor`; JSON stdout，远端 miss 才需 auth，不提供 daemon/purge/refresh/compact |
