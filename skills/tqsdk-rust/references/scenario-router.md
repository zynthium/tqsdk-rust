...
| "历史 HTTP", "history API", "`/v1/history/query`", "coverage", "schema", "Docker 部署 relay" | Read-only CacheOnly history service | `tqsdk-relay` plus optional `tqsdk-cache` publisher | 先读 [`relay-history-http.md`](relay-history-http.md)；调用 `GET /v1/history/{query,coverage,schema}`，部署与快照发布使用 `deploy/docker` 和 `tqsdk-cache snapshot` |
