...
- S16 历史行情回放从“不自然”推进到“勉强”：`tqsdk-task::StrategyReplay`
  ...
  adapter。replay clock/checkpoint、多序列 convenience builder 和完整
...
| 16. 历史行情回放 | 勉强 | 中 | 无 | 无 | 低 | 中 | 局部重构 | `crates/tqsdk-task/examples/api_contract_s16_history_replay_strategy.rs`; `docs/scenarios/api_gaps/api_contract_s16_history_replay_strategy.rs`; `KlineDataSeries::into_market_cache_replay`; `TickDataSeries::into_market_cache_events`; `StrategyReplay`; replay clock/checkpoint/multi-series builder/live-sim-replay environment still gap |
...
4. 多 provider 聚合、完整 live/sim/replay environment、replay clock/checkpoint、live durable cache sink、fake reconnect/deterministic clock 都是新 facade/tooling 层问题，不应下沉到 `tqsdk-core` 或 `tqsdk-session`；本地行情 cache record / JSONL reader-writer / ordered replay foundation 与 history series replay adapter 已落在 `tqsdk-data`，cache replay -> strategy context foundation 已落在 `tqsdk-task`。
