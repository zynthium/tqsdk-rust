...
- S16 历史行情回放从“不自然”推进到“勉强”：`tqsdk-task::StrategyReplay`
  ...
  直接 `DataClient` history series adapter、replay clock/checkpoint 和完整
...
4. 多 provider 聚合、完整 live/sim/replay environment、history series adapter、live durable cache sink、fake reconnect/deterministic clock 都是新 facade/tooling 层问题，不应下沉到 `tqsdk-core` 或 `tqsdk-session`；本地行情 cache record / JSONL reader-writer / ordered replay foundation 已落在 `tqsdk-data`，cache replay -> strategy context foundation 已落在 `tqsdk-task`。
