...
- S21 慢消费者隔离的 bounded fan-out / lag 诊断 / sink policy 子集已经自然表达：`TqStreamBuilder`
  ...
  flush report、WAL fsync policy 和本地 JSONL compaction；durable queue 和跨进程
  sink 恢复仍保留为 gap。
...
| 21. 慢消费者隔离 | 自然 | 低 | 无 | 无 | 低 | 低 | API 微调 | `crates/tqsdk-stream/examples/api_contract_s21_slow_consumer_isolation.rs`; `docs/scenarios/api_gaps/api_contract_s21_slow_consumer_isolation.rs`; `TqStream::spawn_commit_sink`; `TqStream::spawn_commit_sink_with_options`; `CommitSink`; `StreamSinkOptions`; `StreamSinkRetryPolicy`; `StreamSinkHandle`; `StreamSinkStats`; `StreamSinkShutdownReport`; `StreamSinkWalRecord`; `StreamSinkWalFsyncPolicy`; `StreamSinkWalCompaction`; bounded fan-out / typed lag diagnostic / managed commit sink / finite retry / JSONL WAL / fsync policy / local compaction 自然；durable queue, cross-process sink recovery still gap |
...
3. `tqsdk-stream` 的底座方向正确，quote 订阅、动态 quote handle、混合 market event、health snapshot、health status、typed reconnect monitor、graceful shutdown、fan-out capacity、typed lag/error diagnostics、managed commit sink、有限重试、JSONL WAL、WAL fsync policy 和本地 compaction 已有薄 facade；`tqsdk-task::StrategySupervisor` 已提供 transport-neutral typed telemetry/export hook；durable queue、跨进程 sink 恢复和跨进程 daemon orchestration 仍停留在底层组合能力之外。
