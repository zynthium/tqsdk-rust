...
- S18 本地行情缓存读写继续推进但仍为“勉强”：`MarketCacheWriter` /
  ...
  覆盖 stale lock recovery、queue flush progress 和 shutdown report。持续后台
  supervisor、lease heartbeat 与多进程 cache 管理仍是 gap。
...
| 18. 本地行情缓存读写 | 勉强 | 中 | 无 | 无 | 低 | 中 | 局部重构 | `crates/tqsdk-data/examples/api_contract_s18_local_market_cache.rs`; `crates/tqsdk-data/examples/api_contract_s18_cache_maintenance.rs`; `crates/tqsdk-data/examples/api_contract_s18_cache_daemon_foundation.rs`; `crates/tqsdk-data/examples/api_contract_s18_live_market_cache_pipe.rs`; `docs/scenarios/api_gaps/api_contract_s18_local_market_cache.rs`; `MarketCacheWriter`; `MarketCacheReader`; `MarketCacheReplay`; `MarketCacheStreamWriter`; `MarketCacheQueue`; `MarketCacheLock`; `MarketCacheIndex`; `MarketCacheCompaction`; `MarketCacheDaemon`; single-process live stream pipe and process-local cache daemon foundation; background supervisor / lease heartbeat / cross-process cache service still gap |
...
4. 多 provider 聚合、完整 production daemon 和 durable cross-process cache daemon 都是新 facade/tooling 层问题，不应下沉到 `tqsdk-core` 或 `tqsdk-session`；本地行情 cache record / JSONL reader-writer / ordered replay foundation、history series replay adapter、单进程 live stream pipe、JSONL queue、lock lease、index、compaction 与 process-local daemon foundation 已落在 `tqsdk-data`，cache replay -> strategy context foundation、strategy environment/deployment/supervisor foundation、provider-backed TQKQ sim config、replay source builder、replay speed policy、checkpoint store、test clock、fake broker latency、fake broker reconnect 与跨 step partial fill 已落在 `tqsdk-task`。
