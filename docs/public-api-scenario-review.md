...
4. 多 provider 聚合、完整 live/sim/replay environment、历史回放驱动策略、live durable cache sink、fake reconnect/deterministic clock 都是新 facade/tooling 层问题，不应下沉到 `tqsdk-core` 或 `tqsdk-session`；本地行情 cache record / JSONL reader-writer / ordered replay foundation 已落在 `tqsdk-data`。
