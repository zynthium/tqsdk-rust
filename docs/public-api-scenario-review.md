...
- S20 生产守护进程继续推进但仍为“勉强”：`TqStream::health()` 现在返回
  ...
  telemetry/export hook；`tqsdk-stream` 已提供 managed commit sink foundation。
  S20 完成标准不包含 Rust GUI、web helper 或内置 HTTP health/metrics endpoint；
  剩余 gap 是 per-sink retry/storage policy、本地 WAL、完整 reconnect orchestration
  和跨进程 daemon 管理。
- S21 慢消费者隔离的 bounded fan-out / lag 诊断子集已经自然表达：`TqStreamBuilder`
  ...
  lag 诊断；`TqStream::spawn_commit_sink(...)` 已提供 managed commit sink
  foundation、typed stats 和 shutdown flush report；per-sink retry/storage policy
  与本地 WAL 仍保留为 gap。
...
| 21. 慢消费者隔离 | 勉强 | 中 | 无 | 无 | 低 | 低 | 局部重构 | `crates/tqsdk-stream/examples/api_contract_s21_slow_consumer_isolation.rs`; `docs/scenarios/api_gaps/api_contract_s21_slow_consumer_isolation.rs`; `TqStream::spawn_commit_sink`; `CommitSink`; `StreamSinkHandle`; `StreamSinkStats`; `StreamSinkShutdownReport`; bounded fan-out / typed lag diagnostic / managed commit sink foundation 自然；per-sink retry/storage policy and local WAL still gap |
...
| 24. 最小可测试策略 | 勉强 | 中 | 无 | 无 | 低 | 低 | 局部重构 | `crates/tqsdk-task/examples/api_contract_s24_testable_strategy.rs`; `docs/scenarios/api_gaps/api_contract_s24_testable_strategy.rs`; `StrategyTestHarness`; `FakeMarket`; `FakeBroker`; `StrategyTestClock`; `FakeBroker::partial_fills`; `FakeBroker::latency_steps`; `FakeBroker::disconnect_for_steps`; `FakeBrokerConnectionStatus`; durable fixtures and richer broker behavior remain gap |
...
3. `tqsdk-stream` 的底座方向正确，quote 订阅、动态 quote handle、混合 market event、health snapshot、health status、fan-out capacity、typed lag/error diagnostics 和 managed commit sink foundation 已有薄 facade；`tqsdk-task::StrategySupervisor` 已提供 transport-neutral typed telemetry/export hook；per-sink retry/storage policy、本地 WAL 和完整 daemon supervisor 仍停留在底层组合能力之外。
