...
3. local materialization
   ...
   - 已有最薄的 local JSONL queue、lock lease、index、保留策略 compaction、in-place rotation、shutdown flush report 与 process-local supervisor foundation
   - 后续再考虑路径管理型文件导出
...
3. 继续按 history/query -> batch fetch -> materialization/cache foundation -> replay driver 的顺序迭代；当前 history series -> cache replay adapter、live stream pipe、local queue/lock/index/compaction/daemon/supervisor foundation 已完成，后续重点是路径管理型 materialization 和跨进程 cache 管理服务
