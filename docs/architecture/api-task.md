...
- 真实 replay sleep/speed policy、durable checkpoint persistence、多序列 replay builder、fake reconnect / latency / deterministic clock
...
2. 在 `StrategyHost` / `StrategyReplay` 之上继续设计 live / sim / replay
   environment adapter、真实 replay sleep/speed policy、durable checkpoint
   persistence 和多序列 replay builder。
