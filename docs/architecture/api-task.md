...
- HTTP metrics endpoint、完整 reconnect orchestration、跨进程 daemon 管理 / 多 provider environment
...
2. 在 `StrategySupervisor` 之上继续设计 HTTP metrics endpoint、完整 reconnect
   orchestration、持久化 sink lifecycle 和跨进程 daemon 管理。
