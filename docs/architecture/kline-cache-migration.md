...
canonical 日线和分钟线文件必须使用 `TQKLOG01` 追加封装。TQDK v1 / TQMK v5
仍是封装内 payload 的编码身份，不再是可以独立交给普通 reader 的文件布局。
新建、重叠更新、snapshot identity 变更和压实都输出 KLOG；普通 fill 只恢复提交尾部，
不再转换旧 raw 文件。query、coverage、doctor、snapshot publish/reader 均拒绝旧 raw。

旧 standalone 编码只供显式离线迁移使用；payload codec 本身仍用于每个 KLOG 片段，不属于兼容回退。
...
1. 深度验证旧 payload、path identity、coverage 和行；已有 KLOG 必须尾部干净。
2. 在 ROOT 外保存原文件，优先 hardlink，跨文件系统时临时复制后原子发布；同步备份与目录。
3. 在 reader 不会识别的临时路径封装原始字节，不重新编码原始 payload。
4. 对比片段与旧文件逐字节相同，并用新 reader 深度验证。
5. 原子替换 canonical 文件，再同步父目录。

迁移不是跨文件事务。中断后保留已完成文件，使用相同 ROOT/BACKUP 重跑；将被替换的 raw
若已有备份，必须与当前文件逐字节相同，否则拒绝覆盖。已有 KLOG 深验后直接跳过，不重写
...
不参与 snapshot 发布。已有 KLOG 的备份若共享 inode，后续 writer 的 COW 必须保护它。
...
新 manifest 必须声明 `kline-append-v1`。
...
拒绝 raw、新建和覆盖写均输出 KLOG。格式细节见 [history-cache-format.md](history-cache-format.md)。
