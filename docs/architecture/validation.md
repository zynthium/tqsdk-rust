...
| 可选 market relay | `cargo test -p tqsdk-relay --tests` | 覆盖 relay 配置、每日合约集合刷新调度、typed metadata 期货产品发现、上游 `ins_list` 长度防线、下游 market 协议、interest/chart-id 隔离、K 线 `[start,end)` 合成、bootstrap 队列限流、observability、WebSocket loopback 和 upstream tick scaffold |
