...
| relay rolling cache | `cargo test -p tqsdk-data rolling_market_cache`、`cargo test -p tqsdk-relay --lib`、`cargo test -p tqsdk-relay --tests` | 覆盖 10,000 根隔离滚动 TQBN、启动恢复、官方 Kline 优先、显式预热/按需订阅、10 分钟空闲宽限与退订状态机、source epoch、缺口重基线和 writer 指标；5 分钟官方尾端对账的真实上游响应仍须在 credential-gated smoke 中确认。 |
