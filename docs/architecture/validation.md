...
| 官方对象 typed schema | `crates/tqsdk-core/tests/runtime_contract_types.rs`、`crates/tqsdk-core/tests/runtime_contract_reader_surface.rs` | 覆盖 `objs.py` 对象族和 core 补充 diff 对象的 typed schema surface，以及 reader 侧 `decode<T>()` 接入 |
...
6. `cargo test -p tqsdk-stream live_quote_stream_smoke -- --ignored --nocapture`
7. `cargo test -p tqsdk-stream live_quote_stream_with_session_query_smoke -- --ignored --nocapture`
8. `cargo test -p tqsdk-stream live_trade_session_event_smoke -- --ignored --nocapture`
9. `cargo test -p tqsdk-task live_task_host_trade_account_ready_smoke -- --ignored --nocapture`
10. `cargo test -p tqsdk-task live_insert_cancel_guarded_smoke -- --ignored --nocapture`
