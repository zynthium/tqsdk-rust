...
| 远端 tick 缓存补齐 | `crates/tqsdk/src/backtest_remote.rs` 单元测试、`crates/tqsdk/tests/facade_contract.rs` | 覆盖不设默认 batch 墙钟超时、显式超时解析、流式 tick id 区间合并与公开 accumulator 语义一致性、future range 拒绝、idle/exhausted terminal coverage 保护，以及 facade cache warmup / remote-on-miss 契约 |
