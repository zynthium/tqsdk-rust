...
| 可选 market relay | `cargo test -p tqsdk-relay --tests` | 覆盖 relay 配置、动态期货产品发现、下游 market 协议、interest/chart-id 隔离、K 线 `[start,end)` 合成、bootstrap 队列限流、observability、WebSocket loopback 和 upstream tick scaffold |
