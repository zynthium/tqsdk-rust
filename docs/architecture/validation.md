...
| 可选 market relay | `cargo test -p tqsdk-relay --tests` | 覆盖 relay 配置、dry-run 启动自检、结构化启动诊断、HTTP `/health`/`/metrics`、每日合约集合刷新调度、typed metadata 期货产品发现与分批查询、上游 `ins_list` 长度防线和拆分建议、下游 market 协议、interest/chart-id 隔离、K 线 `[start,end)` 合成、tick-ring 冷启动回放、bootstrap 队列限流、observability、WebSocket loopback 和 upstream tick scaffold |
