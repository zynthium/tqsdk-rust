...
| 可选嵌入式监控 | `cargo test -p tqsdk-monitor`、`cargo test -p tqsdk --features monitoring replay_backtest_connect_starts_embedded_monitor --lib`、`cargo check -p tqsdk --features monitoring --examples` | 覆盖低开销 `MonitorSink` / `MonitorRegistry` 聚合、只读 HTTP snapshot、facade `.monitoring(...)` 接入、disabled monitoring no-op，以及 monitoring feature 不破坏默认 facade examples |
