...
普通 `.universe(...)`、`quotes_universe(...)` 和 relay 都解析当前 snapshot；它们不能恢复历史上
...
旧字段 `physical_listing_starts` 在 provider-history proof 下表示第一条可证明数据 membership，
不是法定挂牌日。用户的回测开始日不会让 downloader 向该证据之前猜测数据；对应 kind 的空前缀与
完整性由 terminal cache coverage 独立证明。
