...
| `tqsdk-data` | S18 cache orchestration surfaces: reader manifest, recovery scan/action, writer election/lease, local queue/lock/index, compaction ownership, service/daemon/supervisor | Removed from `crates/tqsdk-data/src/lib.rs` | S18 was narrowed to `MarketCacheEvent` / writer / reader / replay plus `MarketCacheStreamWriter`; former orchestration examples and cross-process sketch are archived under `docs/archive/scenarios/2026-05-03/`. | `rollback` | Keep out of current public API. Reintroduce only via a new user-tooling or independent service design. |
