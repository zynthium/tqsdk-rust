...
| `tqsdk-data` | History series mmap cache surface: `DataClientBuilder`, `HistorySeriesCache`, `HistorySeriesCacheBackend`, `HistorySeriesCacheReport` | `crates/tqsdk-data/src/lib.rs` | Documented in `docs/architecture/api-data.md`, `crates/tqsdk-data/README.md`, `docs/reviews/public-api-scenario-review.md`, and `api_contract_s30_history_series_cache.rs`. | `keep` | Keep as the explicit opt-in Python-compatible history series cache foundation; do not move it into core/session/wait/stream. |
