...
//! `FakeMarket` 和 `FakeBroker`。用户可以在不连接真实服务、不调用 hidden
//! `*_for_test` API 的情况下，用同一套 `StrategyHost` / `StrategyContext`
//! 路径测试 quote 触发下单、全成、拒单和部分成交。
//!
//! Remaining API gap:
//! 当前 test harness 仍是 foundation：fake reconnect、延迟、跨 step
//! 部分成交推进、deterministic clock、持久化恢复和更完整 broker 行为仍未冻结。
//!
//! 理想用户代码草案：
//! ```ignore
//! #[tokio::test]
